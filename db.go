...
type RedisDb struct {
	...
	mu      sync.Mutex
	memUsed uint64 // memUsed is approximate memory usage of the database in bytes across
}
...
func (rdb *RedisDb) Set(key, val string) {
	...
	if old, ok := rdb.store[key]; ok {
		rdb.memUsed -= old.approxMemUsage(key)
	}
	item := Item{Value: val}
	...
	rdb.memUsed += imem
	rdb.store[key] = &item
	log.Printf("set key=%q, memory usage=%d bytes", key, rdb.memUsed)
}
...
func (rdb *RedisDb) Delete(key string) {
	...
	rdb.memUsed -= item.approxMemUsage(key)
	delete(rdb.store, key)
	log.Printf("deleted key=%q, memory usage=%d bytes", key, rdb.memUsed)
}
